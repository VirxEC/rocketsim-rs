...
struct Arenar {
  ...
  Arenar(GameMode game_mode, EArenaConfig arenaConfig, float tick_rate) {
    ArenaConfig config = ArenaConfig{
        ...
        .minPos = Vec(arenaConfig.minPos.x, arenaConfig.minPos.y,
                      arenaConfig.minPos.z),
        .maxPos = Vec(arenaConfig.maxPos.x, arenaConfig.maxPos.y,
                      arenaConfig.maxPos.z),
  ...
  void SetGoalScoreCallback(rust::Fn<void(Arenar &, Team, size_t)> callback,
                            size_t);
  void SetCarBumpCallback(
  ...
  // extra car stuff
  size_t NumCars() const { return a->_cars.size(); }
  ...
  Team GetCarTeam(uint32_t car_id) const { return a->GetCar(car_id)->team; }
  ...
  BallState GetBall() { return a->ball->GetState(); }

  void SetBall(const BallState state) { a->ball->SetState(state); }

  float GetBallRadius() const { return a->ball->GetRadius(); }

  // boost pad stuff

  size_t NumPads() const { return a->_boostPads.size(); }
  ...
  void ResetTickCount() { a->tickCount = 0; }

  uint64_t GetTickCount() const { return a->tickCount; }

  float GetTickRate() const { return 1 / a->tickTime; }

  GameMode GetGameMode() const { return a->gameMode; }

  void ResetToRandomKickoff(int32_t seed = -1) {
    a->ResetToRandomKickoff(seed);
  }

  void Step(uint32_t ticks = 1) { a->Step(ticks); }

  bool IsBallProbablyGoingIn(float maxTime = 2.f, float extraMargin = 0) const {
    return a->IsBallProbablyGoingIn(maxTime, extraMargin);
  }

  bool IsBallScored() const { return a->IsBallScored(); }

  MutatorConfig GetMutatorConfig() const { return a->GetMutatorConfig(); }

  void SetMutatorConfig(MutatorConfig mutatorConfig) {
    a->SetMutatorConfig(mutatorConfig);
  }

  void SetDropshotTilesState(const DropshotTilesState &state) {
    a->SetDropshotTilesState(state);
  }

  const DropshotTilesState &GetDropshotTilesState() const {
    return a->_dropshotTilesState;
  }
};
